...
describe('InsidersVesting', function () {
    ...
    it('should revert initialize when input params incorrect', async function () {
        ...
        await expect(vesting.initialize(superproToken.address, [user1.address, user2.address], [parseEther(200000000), parseEther(200000001)], START)).be.revertedWith('Exceeded tokens limit');
    ...
    it('should forbid transfer if requested more tokens than available', async function () {
        ...
        const unlockedTokens = parseEther(1);
        await expect(vesting.connect(user1).transfer(user4.address, lockedTokens, unlockedTokens)).be.revertedWith('Requested more tokens than unlocked');
    });
